...
######################################################################################
# Set the Schema
fields = [StructField(*field) for field in PRODUCTS_SCHEMA]
schema_stores = StructType(fields)
######################################################################################
# Pulling DataFrame from Minio
df_table_incremental = spark.read.option("header",True).schema(schema_stores).csv("s3a://bronze/sales/products/2022/07/02/09/products.csv")
######################################################################################
# Printing Result
...
df_table_curated.write.option("header","true").csv("s3a://silver/csv/products")
df_table_curated.write.option("compression","snappy").parquet("s3a://silver/curated/products")
df_table_curated.write.format("delta").mode("overwrite").option('overwriteSchema','true').save("s3a://silver/delta/products")
######################################################################################